...
type
  TfrmMain = class(TForm)
    ...
    dtpMyTime: TDateTimePicker;
    lblDate: TLabel;
    dtpMyDate: TDateTimePicker;
  ...
  private
    procedure SendValuetoArduino(AIndex: Integer);
...
procedure TfrmMain.SendValuetoArduino(AIndex: Integer);
begin
  Str := '';
...
begin
  ...
  ComPort.BaudRate := br9600; // Set Default BaudRate
  // ------------------------------------- //
  TrackBarSpeed.Min := 0;
...
var
  ...
  DTStamp: Int64;
begin
  DTValue := DateOf(dtpMyDate.Date) + TimeOf(dtpMyTime.Time);
  Memo.Clear;
  Memo.Lines[0] := 'Set DateTime -> ' + DateTimeToStr(DTValue);
  DTStamp:=DateTimeTounix(DTvalue);
  Str := '';
  Str := 'T' +DtStamp.ToString + sLineBreak;
  ComPort.WriteStr(Str);
